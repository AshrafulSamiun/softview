<template>
    ...
    <fieldset>
        <form id="msform" @submit.prevent="editmode ? updateCustomerProfile() : createCompanyProfile()" @keydown="form.onKeydown($event)">
            <div class="form-card">
                <h1 class="page-head">Customer (Service Provider) Information</h1>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            {            
                  ...
                  Swal.fire({
                    ...
                    confirmButtonText: 'Yes, delete it!'
                      ...
                      this.form.delete('/CustomerServiceProviders/'+this.form.id).then(()=>{
            ...
            {
                let uri = '/CustomerServiceProviders';
            ...
            updateCustomerProfile()
            {
                ...
                this.form.put('/CustomerServiceProviders/'+this.form.id).then(({ data })=>{
                    var response_data=data.split("**");
            ...
            save_stay(type){
                if(this.form.key_position_lavel_data)
                {
                    toast({
                            type: 'danger',
                            title: 'Please Fillup Key Position.'
                        });

                }
                this.form.post('/CustomerServiceProviders') .then(({ data }) => { 
                    var response_data=data.split("**");
                    if(response_data[0]==1)
                    {
                         toast({
            ...
            fetchCustomerProfileUpdate(id)
            {
                ...
                let uri = '/CustomerServiceProviders/'+id+'/edit';
                window.axios.get(uri).then((response) => {
