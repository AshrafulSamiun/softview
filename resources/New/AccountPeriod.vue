                ...
                <div id="content">
                        ...
                        <datatable :columns="columns" :data="rows" :filter-by="filter">
							<template slot-scope="{row}">
								<tr>
				                    ...
				                    <td>{{ row.year_start_date }}</td>
				                    <td>{{ row.year_end_date }}</td>
				                    <td>{{ row.period_name }}</td>
				                    <td>{{ YesArr[row.is_current] }}</td>
				                    <td>
				                        <button class="btn btn-info btn-sm" @click.prevent="showPeriod(row.id)">Show</button>
				                    	<button class="btn btn-primary btn-sm" @click.prevent="editPeriod(row)">Edit</button>
				                    	<button v-show="delete_permission"  class="btn btn-danger btn-sm" @click.prevent="deletePeriod(row.id)">Delete</button>
				                    </td>
				                </tr>
							</template>
						</datatable>
                    ...
                    <div class="form-card" v-if="!list_showable">
                                ...
                                <div class="row align-self-stretch"> 
                                    <div class="col-md-4 col-sm-6 col-xs-12"> 
                                        <div class="form-box-outer">
                                            ...
                                            <select v-model="form.is_current"
                                                name="is_current" 
                                                class="custom-select" 
                                                :class="{ 'is-invalid': form.errors.has('is_current') }">
                                                <option disabled value="">--Select-- </option>
												 <option value="1">Yes</option>
												 <option value="2">No</option>
                                                
                                            </select>
                                            <has-error :form="form" field="is_current"></has-error>

                                            <label class="fieldlabels">Starting Year:</label> 
                                            <select v-model="form.year_start"
                                                name="year_start" 
                                                class="custom-select" 
                                                :class="{ 'is-invalid': form.errors.has('year_start') }">
                                                <option disabled value="">--Select-- </option>
                                            ...
                                            <has-error :form="form" field="year_start"></has-error>

                                            <label class="fieldlabels">Starting Month:</label> 
                                            <div class="position-relative form-group">
                                                 <select v-model="form.year_start_month"
                                                    name="account_type" 
                                                    class="custom-select" 
                                                ...
                                                <has-error :form="form" field="year_start_month"></has-error>
                                            </div>
                                            <label class="fieldlabels">Ending Month:</label> 
                                            <div class="position-relative form-group">
                                                <select v-model="form.year_end_month"
                                                    name="year_end_month"
                                                    class="custom-select" 
                                                    @change="accounting_period_calculate()"
                                                    :class="{ 'is-invalid': form.errors.has('year_end_month') }">
													<option disabled value="">--Select-- </option>
													<option value="1">January</option>
													<option value="2">February</option>
													<option value="3">March</option>
                                                ...
                                                <has-error :form="form" field="year_end_month"></has-error> 
                                            </div>

                                            <label class="fieldlabels" >Period Name:</label>  
                                            <input v-model="form.period_name" type="text" name="period_name" 
                                            ...
                                            <select v-model="form.status_active"  name="status_active"
									        	class="custom-select" :class="{ 'is-invalid': form.errors.has('status_active') }">
											  	<option disabled value="">--Select-- </option>
                        ...
                        <div class="text-center">
                            ...
                            <button :disabled="form.busy || editmode==false || form.posting_status!=0"  type="button" class="btn  btn-primary" style="min-width:110px"  @click.prevent="deletePeriod()">Delete</button>

                            

                            <button :disabled="form.busy || editmode==false || form.posting_status!=0"  type="button" class="btn  btn-primary" style="min-width:110px"  @click="updatePeriod(2)">Save</button>
        ...
        data(){
            return{
            	...
            	columns: [
		            ...
		            { label: 'Year Name', field: 'year_start' },
		            { label: 'Year Start', field: 'year_start_date' },
		            { label: 'Year End', field: 'year_end_date' },
		            { label: 'Period Name', field: 'period_name' },
		            { label: 'Is Current', field: 'is_current' },
